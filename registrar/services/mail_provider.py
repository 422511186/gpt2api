...
class BaseMailProvider:
    ...
    def wait_for(self, mailbox: dict[str, Any], on_message: Callable[[dict[str, Any]], ResultT | None], stop_event: Any = None) -> ResultT | None:
    ...
    def wait_for_code(self, mailbox: dict[str, Any], stop_event: Any = None) -> str | None:
        ...
        return self.wait_for(mailbox, extract_unseen_code, stop_event=stop_event)
...
class TempMailLolProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        ...
        self.api_keys = [line.strip() for line in raw_api_key.split("\n") if line.strip()]
        ...
        # 设置第一个 API key
        self._set_api_key_header()

    def _set_api_key_header(self) -> None:
        """设置当前 API key 到请求头"""
        if self.api_keys:
            self.session.headers["Authorization"] = f"Bearer {self.api_keys[self.api_key_index]}"
    ...
    def _request(self, method: str, path: str, params: dict | None = None, payload: dict | None = None, expected: tuple[int, ...] = (200,)):
        ...
        for attempt in range(len(self.api_keys) if self.api_keys else 1):
            try:
                ...
                # 失败时轮换 API key 重试
                if self.api_keys and attempt < len(self.api_keys) - 1:
                ...
                if self.api_keys and attempt < len(self.api_keys) - 1:
                    ...
                    continue
                raise
        raise RuntimeError("TempMail.lol 所有 API key 均失败")
...
def wait_for_code(mail_config: dict, mailbox: dict, stop_event: Any = None) -> str | None:
    ...
    try:
        return provider.wait_for_code(mailbox, stop_event=stop_event)
    finally:
        provider.close()
