"""
Klein Registrar Service - ChatGPT Account Registration

This service registers ChatGPT accounts and imports them into Klein's account pool.
"""
...
config = {
    ...
    "klein_jwt": "",
    "klein_refresh_token": "",
    "klein_jwt_expires_at": 0,  # Unix timestamp when JWT expires
}
...
def check_stop() -> None:
    if stop_event.is_set():
        raise RegistrationStopped("注册任务已停止")
...
# === FlareSolverr ===
def solve_flare(session: requests.Session, flaresolverr_url: str) -> dict:
    """使用 FlareSolverr 获取 Cloudflare cookies"""
    if not flaresolverr_url:
        return {}

    fs_session = requests.Session()
    fs_session.verify = False

    # 最多重试 3 次
    for attempt in range(3):
        check_stop()
        try:
            resp = fs_session.post(
                f"{flaresolverr_url.rstrip('/')}/v1",
                json={
                    "cmd": "request.get",
                    "url": f"{auth_base}/u/login/password?screen_hint=login_or_signup",
                    "maxTimeout": 60000,
                },
                timeout=90,
            )
            ...
            log(f"FlareSolverr attempt {attempt + 1} failed: {e}", "yellow")
            if attempt < 2:
                if stop_event.wait(5):
                    raise RegistrationStopped("注册任务已停止")
    return {}
...
def exchange_platform_tokens(session: requests.Session, device_id: str, code_verifier: str, consent_url: str) -> dict | None:
    ...
    if not callback_params:
        log(f"[exchange_platform_tokens] 主方案失败，尝试回退方案, continue_url={consent_url[:120]}")
        try:
            check_stop()
            ...
            log(f"[exchange_platform_tokens] 回退方案异常: {e}")
    if not callback_params:
        log("[exchange_platform_tokens] 所有方案均无法提取 OAuth code")
        return None
...
def create_mailbox(username: str | None = None) -> dict:
    """Create mailbox using configured mail provider"""
    check_stop()
    from services.mail_provider import create_mailbox as _create_mailbox
    return _create_mailbox(config["mail"], username)


def wait_for_code(mailbox: dict) -> str | None:
    """Wait for verification code from mailbox"""
    check_stop()
    from services.mail_provider import wait_for_code as _wait_for_code
    return _wait_for_code(config["mail"], mailbox, stop_event=stop_event)


# === Platform Registrar ===
class PlatformRegistrar:
    def __init__(self, proxy: str = "", flaresolverr_url: str = "") -> None:
    ...
    def _solve_flare(self) -> None:
        """Get Cloudflare cookies using FlareSolverr"""
        check_stop()
        cookies = solve_flare(self.session, self.flaresolverr_url)
        for name, value in cookies.items():
            self.session.cookies.set(name, value, domain=".auth.openai.com")
    ...
    def _create_account(self, name: str, birthdate: str, index: int) -> None:
        ...
        if resp is None or resp.status_code not in (200, 302):
            data = _response_json(resp) if resp is not None else {}
            if data.get("message") == "Failed to create account. Please try again.":
                step(index, "创建账号失败提示: 邎箱域名很可能因滥用被封禁，请更换邮箱域名", "yellow")
            detail = f", detail={json.dumps(data, ensure_ascii=False)}" if data else ""
...
def worker(index: int) -> dict:
    """Worker function to register one account"""
    start = time.time()
    registrar = PlatformRegistrar(config["proxy"], config["flaresolverr_url"])
    try:
        check_stop()
        step(index, "任务启动")
        ...
        # Import to Klein account pool
        check_stop()
        import_result = import_to_klein(result)
        if import_result:
            step(index, f"账号导入 Klein 号池成功", "green")
        else:
            step(index, f"账号导入 Klein 号池失败", "red")
            raise RuntimeError("账号导入 Klein 号池失败")

        with stats_lock:
            stats["done"] += 1
            stats["success"] += 1
            base = float(stats.get("start_time") or 0)
            if base <= 0:
                base = start
            avg = (time.time() - base) / max(1, stats["success"])
        log(f'{result["email"]} 注册成功，本次耗时{cost:.1f}s，全局平均每个号注册耗时{avg:.1f}s', "green")
        return {"ok": True, "index": index, "result": result}
    except RegistrationStopped as e:
        cost = time.time() - start
        log(f"任务{index} 已停止，本次耗时{cost:.1f}s", "yellow")
...
def refresh_klein_jwt() -> bool:
    """Refresh Klein JWT using refresh token"""
    if not config["klein_api_base"] or not config["klein_refresh_token"]:
        return False

    try:
        resp = requests.post(
            f"{config['klein_api_base']}/admin/api/v1/auth/refresh",
            headers={"Content-Type": "application/json"},
            json={"refresh_token": config["klein_refresh_token"]},
            timeout=10,
            verify=False,
        )
        ...
        if resp.status_code == 200:
            ...
            config["klein_jwt"] = token.get("access_token", "")
            if token.get("refresh_token"):
                config["klein_refresh_token"] = token["refresh_token"]
            # Set expiry time: current time + (expires_in - 60) seconds (refresh 1 minute before expiry)
            expires_in = token.get("access_expire_in", 7200)
            config["klein_jwt_expires_at"] = time.time() + expires_in - 60
            return bool(config["klein_jwt"])
        if resp.status_code == 404:
            log("JWT refresh failed: 后端缺少 /admin/api/v1/auth/refresh，请先部署新版后台服务", "yellow")
            return False
        else:
            log(f"JWT refresh failed: HTTP {resp.status_code}", "yellow")
            return False
    except Exception as e:
        log(f"JWT refresh exception: {e}", "yellow")
        return False


def ensure_valid_klein_jwt() -> bool:
    """Ensure we have a valid Klein JWT, refresh if needed"""
    if not config["klein_jwt"]:
        return refresh_klein_jwt()

    # Check if JWT is about to expire (within 60 seconds)
    if config["klein_jwt_expires_at"] > 0 and time.time() > config["klein_jwt_expires_at"]:
        log("Klein JWT expired, refreshing...", "yellow")
        if not refresh_klein_jwt():
            return False

    return bool(config["klein_jwt"])


def import_to_klein(result: dict) -> bool:
    """Import registered account to Klein account pool via API"""
    if not config["klein_api_base"]:
        log("Klein API base URL not configured, skip import", "yellow")
        return False

    # Ensure we have valid JWT
    if not ensure_valid_klein_jwt():
        log("Klein JWT not configured or expired, skip import", "yellow")
        return False

    try:
        # Prepare sub2api format for import
        account_item = {
            "name": result["email"],
            "platform": "openai",
            "type": "oauth",
        ...
        resp = requests.post(
            f"{config['klein_api_base']}/admin/api/v1/accounts/import",
            headers={
                "Content-Type": "application/json",
                "Authorization": f"Bearer {config['klein_jwt']}",
            },
            json={
                "format": "sub2api",
                "provider": "gpt",
        ...
        if resp.status_code == 200:
            data = resp.json()
            imported = data.get("data", {}).get("imported", 0)
            return imported > 0
        elif resp.status_code == 401:
            # JWT expired, try refresh and retry once
            log("Import got 401, refreshing JWT and retrying...", "yellow")
            if refresh_klein_jwt():
                resp = requests.post(
                    f"{config['klein_api_base']}/admin/api/v1/accounts/import",
                    headers={
                        "Content-Type": "application/json",
                ...
                if resp.status_code == 200:
                    ...
                    imported = data.get("data", {}).get("imported", 0)
                    return imported > 0
            log(f"Import failed after refresh: HTTP {resp.status_code}", "red")
            return False
        else:
            log(f"Import failed: HTTP {resp.status_code}, {resp.text[:200]}", "red")
            return False
    except Exception as e:
        log(f"Import exception: {e}", "red")
        return False
